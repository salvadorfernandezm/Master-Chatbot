...
export default function ImpactoPage() {
  const { data: session, status } = useSession(); // Añadimos status para saber si está cargando
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [backUrl, setBackUrl] = useState("/buzon");

  useEffect(() => {
    // 1. LÓGICA DE NAVEGACIÓN CORREGIDA (Prioridad Admin)
    const isPublic = window.location.pathname.includes("/buzon/impacto");
    const authDirector = localStorage.getItem("director_authenticated");
    
    if (session) {
      setBackUrl("/admin"); // Si eres tú (Admin), siempre vuelves a tu centro de mando
    } else if (authDirector === "true") {
      setBackUrl("/admin/directora"); // Si no eres tú, pero es la Dire, vuelve a su despacho
    } else {
      setBackUrl("/buzon"); // Por defecto al portal
    }

    // 2. PROTECCIÓN DE RUTA
    if (!isPublic && status !== "loading" && !session && authDirector !== "true") {
        window.location.assign("/admin/directora"); // Si no es nadie, fuera
        return;
    }
  ...
  if (loading || !data) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-emerald-500 font-black animate-pulse">CARGANDO MÉTRICAS...</div>;
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-center mb-10 bg-slate-900 p-10 rounded-[3rem] text-white shadow-2xl border-b-8 border-emerald-500">
          <div>
            ...
            <p className="text-slate-400 text-xs font-bold uppercase mt-1 italic">Transparencia y Resultados</p>
          </div>
          {!isPublic && (
  ...
  return (
    <div className="space-y-4 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 shadow-inner">
      <div className="flex justify-between items-start">
        <div><span className="text-[9px] font-black uppercase text-slate-400 block mb-1">{label}</span><span className="text-[14px] font-black text-slate-800 block leading-tight">{name || "Pendiente"}</span></div>
        <div className="text-right"><span className="text-2xl font-black text-slate-900">{efficiency}%</span><p className="text-[8px] font-bold uppercase text-slate-400">Eficiencia</p></div>
      </div>
      <div className="grid grid-cols-2 gap-2 text-center">
          <div className="bg-white p-2 rounded-xl border border-slate-100"><p className="text-[8px] font-black text-slate-400 uppercase">Recibidos</p><p className="text-sm font-black text-slate-700">{stats?.recibidos || 0}</p></div>
          <div className="bg-white p-2 rounded-xl border border-slate-100"><p className="text-[8px] font-black text-emerald-400 uppercase">Resueltos</p><p className="text-sm font-black text-emerald-600">{stats?.resueltos || 0}</p></div>
      </div>
      ...
      {(stats?.apelados > 0 || stats?.noAtendidos > 0) && (
        <div className="flex gap-2 pt-1">
          {stats.apelados > 0 && <span className="text-[9px] font-bold bg-red-100 text-red-600 px-2 py-0.5 rounded-full uppercase">⚠️ {stats.apelados} Apelado(s)</span>}
          {stats.noAtendidos > 0 && <span className="text-[9px] font-bold bg-black text-white px-2 py-0.5 rounded-full uppercase">⌛ {stats.noAtendidos} Retraso(s)</span>}
        </div>
      )}
    </div>
  );
