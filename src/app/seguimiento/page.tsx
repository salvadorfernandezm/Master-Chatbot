      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/10 shadow-2xl mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSearch} disabled={loading} className="bg-emerald-600 hover:bg-emerald-500 px-6 rounded-2xl font-bold">{loading ? "..." : "🔎"}</button>
          </div>
          ...
          <div className="bg-slate-900 rounded-[2.5rem] border-b-8 border-emerald-500 overflow-hidden shadow-2xl">
            <div className="p-8 space-y-6">
              <div className="flex justify-between items-center border-b border-white/5 pb-4">
                ...
                <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase ${ticket.status === 'RESUELTO' ? 'bg-emerald-500 text-black' : ticket.status === 'APELADO' ? 'bg-red-600 text-white animate-pulse' : 'bg-amber-500 text-white'}`}>{ticket.status}</span>
              </div>
              <div className="bg-black/20 p-4 rounded-2xl border border-white/5">
                <p className="text-[10px] font-black text-slate-500 mb-2 uppercase">Tu reporte:</p>
              ...
              <div className="bg-emerald-500/5 p-6 rounded-3xl border border-emerald-500/20">
                <p className="text-[10px] font-black uppercase text-emerald-500 mb-2">Respuesta recibida:</p>
                <p className="text-sm text-white leading-relaxed whitespace-pre-wrap">{ticket.authorityResponse || "En proceso de revisión..."}</p>
                {ticket.attachments?.filter((a:any) => a.type === 'AUTHORITY').length > 0 && (
                  <div className="mt-4 pt-4 border-t border-emerald-500/10">
                    <p className="text-[9px] font-black text-emerald-500 uppercase mb-2">Comprobantes adjuntos:</p>
                    <div className="flex flex-wrap gap-2">
                      {ticket.attachments.filter((a:any) => a.type === 'AUTHORITY').map((file:any) => (
                        <a key={file.id} href={file.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 px-4 py-3 rounded-2xl text-xs font-bold text-emerald-400 hover:bg-emerald-500/20 transition-all">
              ...
              <div className="mt-8 pt-6 border-t border-white/5">
                {ticket.status === 'APELADO' ? (
                  <div className="p-4 bg-red-500/10 rounded-2xl border border-red-500/20 text-red-400 font-bold text-xs uppercase text-center">Caso en apelación ante Dirección.</div>
                ) : ticket.studentResolved || voted ? (
                  <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 text-emerald-400 font-bold text-xs uppercase text-center">✓ Caso finalizado.</div>
                ) : ticket.status === 'RESUELTO' ? (
                  <div className="space-y-4">
                    {!showAppealBox ? (
                      <div className="flex gap-4">
                        <button onClick={handleSatisfied} className="flex-1 bg-emerald-600 py-4 rounded-2xl font-black uppercase text-[10px]">CONFORME</button>
                        <button onClick={() => setShowAppealBox(true)} className="flex-1 bg-red-600 py-4 rounded-2xl font-black uppercase text-[10px]">INCONFORME</button>
                      </div>
                    ) : (
                      <div className="space-y-4 animate-in slide-in-from-top-2">
                        <textarea value={appealReason} onChange={(e) => setAppealReason(e.target.value)} className="w-full bg-black border-2 border-red-900/30 p-4 rounded-2xl text-sm text-white" rows={3} placeholder="Explica por qué no estás de acuerdo..." />
                        <div className="flex gap-2">
                          <button onClick={handleSendAppeal} disabled={sendingAppeal} className="flex-1 bg-red-600 py-4 rounded-2xl font-black uppercase text-[10px]">{sendingAppeal ? "..." : "Enviar Apelación"}</button>
                          <button onClick={() => setShowAppealBox(false)} className="px-6 bg-slate-800 py-4 rounded-2xl font-black uppercase text-[10px]">X</button>
                        </div>
                      </div>
                    )}
                  </div>
